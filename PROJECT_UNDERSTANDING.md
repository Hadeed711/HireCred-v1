...
This is the heart of the product. When a profile is saved/updated, the backend sends the full profile data to Gemini AI with a carefully designed prompt.
...
Gemini reads this and extracts:
```json
{
  "skill_rating": 7.5,
...
1. **AI parse** — Gemini extracts from the query: required skills, intent signals (reliability, experience level, trust priority)
...
When a new appreciation is submitted, Gemini analyzes the full set of appreciations for that candidate:
...
Candidate fills profile
        ↓
Backend sends profile to Gemini AI
        ↓
Gemini returns HireCred Score JSON
        ↓
...
Client writes appreciation text
        ↓
Gemini converts text → structured ratings
        ↓
...
1. **Why Python backend?** Clean, fast to write AI integration with the Google Generative AI SDK, great for prompt engineering logic.
2. **Why Neon DB (serverless PostgreSQL)?** No local setup, free tier, scales to zero, works perfectly with Python + asyncpg/psycopg2.
3. **Why Gemini API?** Structured outputs and strong reasoning for nuanced tasks like fraud detection.
